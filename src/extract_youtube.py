...
def extract_youtube_data(
    youtube: googleapiclient, video_id: str, all_comments=[], token=""
) -> pd.DataFrame:
    ...
    video_response = (
        ...
        .list(part="snippet", videoId=video_id, pageToken=token)
        .execute()
    )
    ...
    if "nextPageToken" in video_response:
        return extract_youtube_data(
            youtube, video_id, all_comments, video_response["nextPageToken"]
        )
    else:
...
def load_youtube_data(df: pd.DataFrame) -> None:
...
def transform_youtube_data(df: pd.DataFrame, video_id: str) -> pd.DataFrame:
    df2 = df.copy()
