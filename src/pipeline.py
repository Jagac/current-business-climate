...
with DAG(
    ...
    task_reddit_etl = PythonOperator(task_id="reddit_etl", python_callable=reddit_main)

    task_youtube_etl = PythonOperator(
        task_id="youtube_etl", python_callable=youtube_main
    )

    combine_tables = PythonOperator(
        task_id="combine_tables", python_callable=append_tables
    )
